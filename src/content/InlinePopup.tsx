...
const linkHoverHandlers = withLinkHoverHandlers();

const withSolidButtonHoverHandlers = () => {
  return {
...
const withGhostButtonHoverHandlers = () => {
  return {
    onMouseEnter: (event: React.MouseEvent<HTMLButtonElement>) => {
      if (event.currentTarget.disabled) return;
...
const solidButtonHoverHandlers = withSolidButtonHoverHandlers();
const ghostButtonHoverHandlers = withGhostButtonHoverHandlers();
...
const EntryLink = (props: {
  ...
  const { entry, externalIconUrl, linkStyle, titleStyle, iconSize = 12 } = props;
  return (
    <a
...
const TopMatchBlock = (props: {
  ...
  return (
    ...
    >
      ...
      {entry.Description && <DescriptionBlock value={String(entry.Description)} />}
...
export const InlinePopup = (props: InlinePopupProps) => {
  const { matches, logoUrl, externalIconUrl, onClose, onSuppressSite } = props;
